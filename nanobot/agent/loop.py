...
async def _emit_progress_callback(
    callback: Callable[..., Awaitable[None]],
    content: str,
    **kwargs: Any,
) -> None:
    """Call a progress callback with backward-compatible keyword filtering."""
    if not kwargs:
    ...
    if not filtered and not content:
        return
    await callback(content, **filtered)
...
class _LoopHook(AgentHook):
    ...
    async def before_execute_tools(self, context: AgentHookContext) -> None:
        if self._on_progress:
            if not self._on_stream:
                ...
                if thought:
                    await _emit_progress_callback(self._on_progress, thought)
    ...
    async def after_iteration(self, context: AgentHookContext) -> None:
        if self._on_progress and context.tool_calls and context.tool_events:
            ...
            if tool_events:
                await _emit_progress_callback(self._on_progress, "", tool_events=tool_events)
        u = context.usage or {}
        logger.debug(
